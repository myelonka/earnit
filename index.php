<html>
    <head>
        <title>earnit - homepage</title>
		<meta charset="utf-8">
		<link rel="stylesheet" href="main.css">
		<link rel='icon' href='img/favicon.png'>
    </head>
	<body>
		
		<?php
			include('config.php');
			include('header.php');
		?>
		
		<div class="page-container">
			<div>1</div>
			<div>2</div>
			<div>3</div>
			<div>4</div>
			<div>5</div>
			<div>6</div>
			<div>7</div>
			<div>8</div>
			<div>9</div>
			<div>10</div>
			<div>11</div>
			<div>12</div>
			<div class="col-12"><h1>Lorem ipsum dolor sit amet.</h1></div>
			<div class="col-6"><h2>Lorem ipsum dolor sit amet.</h2></div>
			<div class="col-6" style="grid-column: 7/13"><h2>Lorem ipsum dolor sit amet.</h2></div>
			<div class="col-3"><h3>Lorem ipsum dolor sit amet.</h3></div>
			<div class="col-3" style="grid-column: 4/7"><h3>Lorem ipsum dolor sit amet.</h3></div>
			<div class="col-3" style="grid-column: 7/10"><h3>Lorem ipsum dolor sit amet.</h3></div>
			<div class="col-3" style="grid-column: 10/13"><h3><a href="js/landing-page.php">Go to landing page >></a></h3></div>
		</div>
        <?php 
			include('footer.php');
		?>
	</body>
</html>